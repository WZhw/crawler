...
int main(int argc, char *argv[])
{
	...
	if (argc != 4) {
		printf
		    ("Usage:\n"
		     "%s 127.0.0.1 80 url.txt\n",
		     argv[0]);
		return 0;
	}
	chdir("downpage");
	putlinks2queue(argv + 1, argc - 3);	/*把用户命令行提供的link放入待爬取url队列 */
	...
	while (1) {
		n = epoll_wait(epfd, events, 256, 2000);	/*等待sockfd可读，即等待http response */
...
void addurl2queue(Url * url)
{
	...
	**/
	if( strcmp(url->ip, "127.0.0.1") != 0){
        return;
	}
	else{
        fprintf(stderr,"Url内容完整。domain=%s\tpath=%s\tip=%s\n",url->domain,url->path,url->ip);
        if (url_queue.size() >= MAX_QUEUE_SIZE)	/*如果队列已满，就忽略该url */
            return;
        pthread_mutex_lock(&queue_lock);
        url_queue.push(url);
        pthread_mutex_unlock(&queue_lock);
		 ...
		 pthread_attr_t * pAttr)
{
	...
	if (pAttr == NULL) {
		...
		pthread_attr_setstacksize(pAttr, 1024 * 1024);	/*1M的堆栈 */
